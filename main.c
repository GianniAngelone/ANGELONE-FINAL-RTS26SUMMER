...
static void btn_task_sem(void *arg)
{
    for (;;) {
        if (xSemaphoreTake(btn_sem, portMAX_DELAY) == pdTRUE) {
             ...
             */
            ESP_LOGI(TAG, "[sem] tamper event #%lu  latency=%lld us (max=%llu)",
...
static void btn_task_notif(void *arg)
{
    for (;;) {
        ...
        ESP_LOGI(TAG, "[notif] tamper event #%lu  latency=%lld us (max=%llu) "
                      "(notif count=%lu)",
                 (unsigned long)presses_observed,
                 (long long)lat,
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "==== App 3 [Hardware Security] starting — ISR + bottom-half ====");
